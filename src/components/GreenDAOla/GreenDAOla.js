...
const GreenDAOla = () => {
  ...
  const approve = async () => {
    ...
    const signer = provider.getSigner(address);
    if(buyOrSell == 1) {
      const DAITOKEN = new ethers.Contract("0x001B3B4d0F3714Ca98ba10F6042DaEbF0B1B7b6F",DAIABI, signer);
      const tx = await DAITOKEN.approve("0x662CEA353084b26177454aE8D1F4cDD5470594CF", "0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
      const receipt = await tx.wait();
      if(receipt.status == 0) return;
      ...
      const GreenDaoToken = new ethers.Contract("0xaf243F40B5984b0E4fb89e68eeAD5D01Bac8A7c6" ,GreenDaoTokenABI, signer);
      const tx = await GreenDaoToken.approve("0x662CEA353084b26177454aE8D1F4cDD5470594CF", "0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
      const receipt = await tx.wait();
      if(receipt.status == 0) return;
  ...
  const Swap = async () => {
    const provider = new ethers.providers.Web3Provider( window.ethereum, "any" );
    const signer = provider.getSigner(address);
    const GreenDaoDEX = new ethers.Contract("0x662CEA353084b26177454aE8D1F4cDD5470594CF",GreenDAODEXABI,signer);
  ...
  const confirmButtonClick = async () => {
    const provider = new ethers.providers.Web3Provider( window.ethereum, "any" );
    const DAITOKEN = new ethers.Contract("0x001B3B4d0F3714Ca98ba10F6042DaEbF0B1B7b6F",DAIABI,provider);
    const GreenDaoToken = new ethers.Contract("0xaf243F40B5984b0E4fb89e68eeAD5D01Bac8A7c6" ,GreenDaoTokenABI, provider);
    const GreenDaoDEX = new ethers.Contract("0x662CEA353084b26177454aE8D1F4cDD5470594CF",GreenDAODEXABI,provider);
    ...
    if(buyOrSell == 1) {
      const allowanceOfDAI = await DAITOKEN.allowance(accounts[0], GreenDaoDEX.address);
      ...
      const allowanceOfGreenDAO = await GreenDaoToken.allowance(accounts[0], GreenDaoDEX.address);
  ...
  useEffect(() => {
    async function setApprove(){
      ...
      if(buyOrSell == 2) {
        ...
        const allowanceOfDAI = await GreenDaoToken.allowance(accounts[0], GreenDaoDEX.address);
        console.log(allowanceOfDAI.toString());
    ...
    <div className="p-5 text-center token-item mt-[100px]">
      ...
      >
        <Col>
          ...
          <Row className="mt-4">
            <Col className="  ">
              <p>Stake Amount : </p>
              <Button onClick={stake_token} variant="success" className="w-100">
                STAKE
              </Button>
            </Col>
            <Col  className=" ">
              <input className="form-control m-2" type = "number" onChange={(e) => {setStateAmount(e.target.value)}}/>
              <Button onClick={unstake_token} variant="danger" className="w-100">
                UNSTAKE
              </Button>
            </Col>
          </Row>
     ...
     <Modal show={show} onHide={handleClose} animation={false}>
        ...
        <div class="input-group mb-3">
          ...
          <input type="number" min={0} defaultValue={0} id="quantity" class="form-control" aria-label="Quantity of Token" onChange={(e) => {setAmount(e.target.value)}}/>
        </div>
        </Modal.Body>
        <Modal.Footer>
          <Button variant="success" onClick={()=>{if(document.getElementById('quantity').value == '0'){alert('quantity needed~!')}else{confirmButtonClick()} }}>
            Confirm
          </Button>
      ...
      <Modal show={show_approve} onHide={approveClose} animation={false}>
        ...
        <Modal.Footer>
          <Button variant="success" id="approve_button" onClick={()=>{approve()}} >
            Approve
          </Button>
          <Button variant="primary" id="swap_button" onClick={()=>{Swap()}} > 
            Swap
          </Button>
        </Modal.Footer>
      </Modal>
